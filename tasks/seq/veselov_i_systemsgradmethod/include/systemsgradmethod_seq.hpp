...
class SystemsGradMethodSeq : public ppc::core::Task {
  ...
  std::vector<double> SLEgradSolver(const std::vector<double> &A,
                                    const std::vector<double> &b, int n,
                                    double tol = 1e-6);
  double dotProduct(const std::vector<double> &a, const std::vector<double> &b);
  std::vector<double> matrixVectorProduct(const std::vector<double> &A,
                                          const std::vector<double> &x, int n);
  void normalize(std::vector<double> &A);

public:
  explicit SystemsGradMethodSeq(std::shared_ptr<ppc::core::TaskData> taskData_)
