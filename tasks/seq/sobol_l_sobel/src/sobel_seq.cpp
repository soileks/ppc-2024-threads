...
bool sobol::Sobel_seq::validation() {
  internal_order_test();
...
bool sobol::Sobel_seq::pre_processing() {
  internal_order_test();
...
bool sobol::Sobel_seq::post_processing() {
  internal_order_test();
  for (int i = 0; i < width * height; i++) {
...
void sobol::Sobel_seq::process_pixel(int i, int j) {
