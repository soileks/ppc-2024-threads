...
TEST(shokurov_d_convex_hull_stl, test_pipeline_run) {
  // Create data
  ...
  // Create Task
  auto test = std::make_shared<shokurov_d_convex_hull_stl::ConvexHullShokurov>(taskDataSeq);
...
TEST(shokurov_d_convex_hull_stl, test_task_run) {
  // Create data
  std::vector<std::pair<double, double>> in;
  std::vector<std::pair<double, double>> ans;
  ans.emplace_back(0, 0);
