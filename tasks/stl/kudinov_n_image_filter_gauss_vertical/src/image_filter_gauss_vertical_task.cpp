...
Image Image::gauss_filtered(const GaussKernel& gauss_kernel) const {
  ...
  auto process_columns = [&](std::size_t start_x, std::size_t end_x) {
  ...
  for (std::size_t i = 0; i < threads.size(); i++) {
    std::size_t start_x = i * this->_width / threads.size();
    std::size_t end_x = (i == num_threads - 1) ? this->_width : (i + 1) * this->_width / threads.size();
    threads[i] = std::thread(process_columns, start_x, end_x);
  }
