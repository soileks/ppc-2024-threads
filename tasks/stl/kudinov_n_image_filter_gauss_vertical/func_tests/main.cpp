...
#include "stl/kudinov_n_image_filter_gauss_vertical/include/image_filter_gauss_vertical_task.hpp"

using namespace KudinovSTL;

TEST(kudinov_n_image_filter_gauss_vertical_stl, validation_error) {
  // Create data
  std::size_t radius = 1;
  double sigma = -23.3;
...
TEST(kudinov_n_image_filter_gauss_vertical_stl, square_small_image) {
  // Create data
  std::size_t radius = 1;
  double sigma = 1.6;
  auto gauss_kernel = GaussKernel(radius, sigma);
...
TEST(kudinov_n_image_filter_gauss_vertical_stl, non_square_small_image) {
  // Create data
  std::size_t radius = 1;
  double sigma = 1.8;
  auto gauss_kernel = GaussKernel(radius, sigma);
...
TEST(kudinov_n_image_filter_gauss_vertical_stl, square_medium_image) {
  // Create data
  std::size_t radius = 2;
  double sigma = 1.8;
  auto gauss_kernel = GaussKernel(radius, sigma);
...
TEST(kudinov_n_image_filter_gauss_vertical_stl, non_square_medium_image) {
  // Create data
  std::size_t radius = 2;
  double sigma = 2.0;
  auto gauss_kernel = GaussKernel(radius, sigma);
...
TEST(kudinov_n_image_filter_gauss_vertical_stl, square_large_image) {
  // Create data
  std::size_t radius = 3;
  double sigma = 1.6;
  auto gauss_kernel = GaussKernel(radius, sigma);
