...
bool SobelTaskStlVolodin::pre_processing() {
  ...
  try {
    ...
    for (int t = 0; t < numThreads; ++t) {
        ...
        threads[t] = std::thread(std::bind(&SobelTaskStlVolodin::copyRange, this, startIdx, endIdx, reinterpret_cast<int*>(taskData->inputs[0]), sourceImage.data()));
    }
...
bool SobelTaskStlVolodin::post_processing() {
  ...
  try {
    ...
    for (int t = 0; t < numThreads; ++t) {
        int startIdx = t * elementsPerThread;
        int endIdx = (t == numThreads - 1) ? (width_ * height_) : startIdx + elementsPerThread;
        threads[t] = std::thread(std::bind(&SobelTaskStlVolodin::copyRange, this, startIdx, endIdx, resultImage.data(), reinterpret_cast<int*>(taskData->outputs[0])));
    }
