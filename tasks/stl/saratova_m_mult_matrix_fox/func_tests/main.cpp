...
TEST(Saratova_M_Mult_Matrix_Fox, Validation_Test) {
  ...
  SaratovaTaskSTL saratovaTaskSTL(taskDataPar);
  ASSERT_EQ(saratovaTaskSequential.validation(), false);
  ASSERT_EQ(saratovaTaskSTL.validation(), false);
}
...
TEST(Saratova_M_Mult_Matrix_Fox, Identity_Mult_On_Another_Matrix) {
  ...
  SaratovaTaskSTL saratovaTaskSTL(taskDataPar);
  ASSERT_EQ(saratovaTaskSTL.validation(), true);
  saratovaTaskSTL.pre_processing();
  saratovaTaskSTL.run();
  saratovaTaskSTL.post_processing();
