...
TEST(stl_shishkina_v_gauss_block_stl_perf_test, test_pipeline_run) {
  int height = 1500;
  int width = 1500;
  int min = 40;
  int max = 180;
  ...
  // Create data
  std::vector<int> in = getPicture3(height, width, min, max);
  ...
  // Create Task
  auto testTaskSTL = std::make_shared<LinearFilteringGauss>(taskDataSeq);
...
TEST(stl_shishkina_v_gauss_block_stl_perf_test, test_task_run) {
  int height = 1500;
  int width = 1500;
  int min = 40;
  int max = 180;
  ...
  // Create data
  std::vector<int> in_ = getPicture3(height, width, min, max);
