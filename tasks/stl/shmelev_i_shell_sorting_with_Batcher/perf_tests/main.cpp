...
TEST(shmelev_i_shell_sorting_with_Batcher, pipeline_run) {
  ...
  // Create data
  std::vector<int> in = ShmelevTaskSTL::generate_random_vector(count, 1, 1024);
  ...
  // Create Task
  auto testTaskSequential = std::make_shared<ShmelevTaskSTL>(taskDataSeq);
  ...
  ASSERT_TRUE(ShmelevTaskSTL::sorted(out));
}
