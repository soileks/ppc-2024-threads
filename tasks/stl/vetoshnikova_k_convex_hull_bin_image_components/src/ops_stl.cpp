...
#include "stl/vetoshnikova_k_convex_hull_bin_image_components/include/ops_stl.hpp"

#include <thread>

using namespace std::chrono_literals;

bool vetoshnikova_stl::ConstructingConvexHullSeq::pre_processing() {
  internal_order_test();
...
bool vetoshnikova_stl::ConstructingConvexHullSeq::validation() {
  internal_order_test();
  return taskData->inputs_count[0] > 0 && taskData->inputs_count[1] > 0;
}

bool vetoshnikova_stl::ConstructingConvexHullSeq::run() {
  internal_order_test();
  markingComponent();
...
bool vetoshnikova_stl::ConstructingConvexHullSeq::post_processing() {
  internal_order_test();
...
int vetoshnikova_stl::orientation(Point p, Point q, Point r) {
  int val = (q.x - p.x) * (r.y - q.y) - (r.x - q.x) * (q.y - p.y);
  return val;
}

void vetoshnikova_stl::ConstructingConvexHullSeq::convexHull(int label) {
...
void vetoshnikova_stl::ConstructingConvexHullSeq::markingComponent() {
  int label = 2;
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
...
bool vetoshnikova_stl::ConstructingConvexHullSTL::pre_processing() {
  internal_order_test();
  h = taskData->inputs_count[0];
  w = taskData->inputs_count[1];
  img.resize(h);
...
bool vetoshnikova_stl::ConstructingConvexHullSTL::validation() {
  internal_order_test();
  return taskData->inputs_count[0] > 0 && taskData->inputs_count[1] > 0;
}

bool vetoshnikova_stl::ConstructingConvexHullSTL::run() {
  internal_order_test();
  markingComponent();
  ...
  std::vector<std::vector<int>> convexHulls(numComponents);

  auto computeConvexHull = [&](int start, int end) {
    for (int i = start; i < end; ++i) {
      std::vector<int> c = convexHull(PointsComponents[i]);
      convexHulls[i] = std::move(c);
    }
  };

  int numThreads = std::thread::hardware_concurrency();
  std::vector<std::thread> threads;
  int chunkSize = (numComponents + numThreads - 1) / numThreads;

  for (int thread = 0; thread < numThreads; ++thread) {
    int start = thread * chunkSize;
    int end = std::min(start + chunkSize, numComponents);
    if (start < end) {
      threads.emplace_back(computeConvexHull, start, end);
    }
  }
...
bool vetoshnikova_stl::ConstructingConvexHullSTL::post_processing() {
  internal_order_test();
  if (!hull.empty()) {
    for (size_t i = 0; i < hull.size(); i++) reinterpret_cast<int*>(taskData->outputs[0])[i] = hull[i];
  }
...
void vetoshnikova_stl::ConstructingConvexHullSTL::markingComponent() {
  int label = 2;
...
std::vector<int> vetoshnikova_stl::convexHull(std::vector<Point>& points) {
