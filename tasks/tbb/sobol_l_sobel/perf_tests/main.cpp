...
TEST(sobol_l_sobel_tbb, test_pipeline_run) {
  int width = 3000;
  int height = 3000;
  ...
  // Создание задачи
  std::shared_ptr<sobol::Sobel_tbb> sobel = std::make_shared<sobol::Sobel_tbb>(taskData, width, height);
...
TEST(sobol_l_sobel_tbb, test_task_run) {
  int width = 3000;
  int height = 3000;
