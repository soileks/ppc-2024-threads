...
TEST(lebedeva_ksenia_bitwise_sort_double_batcher, arr) {
  ...
  // Create Task
  LebedevaKTbb::RadixSortDoubleWithBatcherSequential testRDSBSequential(RSDBDataSeq);
  ...
  RSDBDataTbb->inputs.emplace_back(reinterpret_cast<uint8_t *>(arr.data()));
  RSDBDataTbb->inputs_count.emplace_back(arr.size());
  ...
  // Create Task
  LebedevaKTbb::RadixSortDoubleWithBatcherTbbParallel testRDSBtbbParallel(RSDBDataTbb);
...
TEST(lebedeva_ksenia_bitwise_sort_double_batcher, test_random) {
  // Create data
  std::vector<double> arr = LebedevaKTbb::randomVector(1000, -1000000, 1000000);
  // Create data
  std::vector<double> out_seq(arr.size());
