...
class HoareSortWBatcherMergeTBB : public ppc::core::Task {
 public:
  explicit HoareSortWBatcherMergeTBB(std::shared_ptr<ppc::core::TaskData> taskData_) : Task(std::move(taskData_)) {}
  bool pre_processing() override;
  bool validation() override;
  bool run() override;
  bool post_processing() override;
