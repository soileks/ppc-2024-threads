...
#include "tbb/kazantsev_e_shtrassen/include/ops_tbb.hpp"

TEST(kazantsev_e_matmul_strassen_tbb_perf, test_pipeline_run) {
  const int n = 128;
  ...
  // Create Task
  auto testTaskSequential = std::make_shared<MatMulStrassenTbb>(taskDataSeq);
...
TEST(kazantsev_e_matmul_strassen_tbb_perf, test_task_run) {
  const int n = 128;
