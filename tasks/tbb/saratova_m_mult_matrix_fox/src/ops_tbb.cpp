...
void saratova_tbb::GenerateIdentityMatrix(double* matrix, int size, double scale) {
...
void saratova_tbb::CreateIdentityMatrix(double* matrix, int size, double scale) {
  std::fill(matrix, matrix + size * size, 0.0);
  for (int i = 0; i < size; ++i) {
...
void saratova_tbb::FillRandomValues(double* matrix, int size) {
...
bool saratova_tbb::SaratovaTaskSequential::validation() {
  internal_order_test();
...
bool saratova_tbb::SaratovaTaskSequential::pre_processing() {
  internal_order_test();
...
bool saratova_tbb::SaratovaTaskSequential::run() {
  internal_order_test();
  try {
...
bool saratova_tbb::SaratovaTaskSequential::post_processing() {
  internal_order_test();
  return true;
}
