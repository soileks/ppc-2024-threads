...
bool CRSComplexMult_tbb::validation() {
  internal_order_test();
...
bool CRSComplexMult_tbb::pre_processing() {
  internal_order_test();
  // convert pointers
  A = reinterpret_cast<matrix_CRS*>(taskData->inputs[0]);
  B = reinterpret_cast<matrix_CRS*>(taskData->inputs[1]);
...
bool CRSComplexMult_tbb::run() {
  ...
  oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, A->n), [&](const oneapi::tbb::blocked_range<int>& r) {
...
bool CRSComplexMult_tbb::post_processing() {
  internal_order_test();
  return true;
}
