...
std::vector<int> shmelev_tbb::create_random_sequence(int size, int min, int max) {
...
bool shmelev_tbb::is_sorted(std::vector<int> input) { return std::is_sorted(input.begin(), input.end()); }

bool shmelev_tbb::ShmelevTaskSequential::pre_processing() {
  internal_order_test();
...
bool shmelev_tbb::ShmelevTaskSequential::validation() {
  internal_order_test();
  return taskData->outputs_count[0] == taskData->inputs_count[0];
}

bool shmelev_tbb::ShmelevTaskSequential::run() {
  internal_order_test();
  batcherMergeSort(0, sequence.size() - 1);
  return true;
}

bool shmelev_tbb::ShmelevTaskSequential::post_processing() {
  internal_order_test();
...
void shmelev_tbb::ShmelevTaskSequential::shellSort() {
  int n = sequence.size();
...
void shmelev_tbb::ShmelevTaskSequential::batcherMergeSort(int left, int right) {
  shellSort();
  if (right > left) {
...
void shmelev_tbb::ShmelevTaskSequential::mergeSequences(int left, int middle, int right) {
...
void shmelev_tbb::ShmelevTaskTbb::shellSort() {
  int n = sequence.size();
  for (int gap = n / 2; gap > 0; gap /= 2) {
    for (int i = gap; i < n; i++) {
      int temp = sequence[i];
...
void shmelev_tbb::ShmelevTaskTbb::batcherMergeSort(int left, int right) {
  shellSort();
  if (right > left) {
    int middle = left + (right - left) / 2;
    tbb::parallel_invoke(
...
void shmelev_tbb::ShmelevTaskTbb::mergeSequences(int left, int middle, int right) {
