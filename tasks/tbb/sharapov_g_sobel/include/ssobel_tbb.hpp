...
class SSobelTbb : public ppc::core::Task {
 public:
  explicit SSobelTbb(std::shared_ptr<ppc::core::TaskData> taskData_) : Task(std::move(taskData_)) {}
  ...
  static std::vector<GrayScale> SobelOperatorTbb(const std::vector<GrayScale>& grayImage, size_t width, size_t height);
