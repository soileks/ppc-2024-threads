...
TEST(kirillov_m_strassen_tbb_perf_tests, test_pipeline_run) {
  const int n = 128;
  ...
  // Create Task
  auto strassenMatrixMultParallelTbb = std::make_shared<StrassenMatrixMultParallelTBB>(taskDataSeq);
  ...
  auto perfAnalyzer = std::make_shared<ppc::core::Perf>(strassenMatrixMultParallelTbb);
...
TEST(kirillov_m_strassen_tbb_perf_tests, test_task_run) {
  const int n = 128;
