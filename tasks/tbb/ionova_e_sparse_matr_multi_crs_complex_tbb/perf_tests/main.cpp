...
TEST(ionova_e_sparse_matr_multi_crs_complex_tbb, test_pipeline_run) {
  size_t p = 500;
  size_t q = 500;
  size_t r = 500;
  ...
  // Create Task
  auto testTaskTbb = std::make_shared<SparseMatrixComplexMultiParallelTbb>(taskDataSeq);
...
TEST(ionova_e_sparse_matr_multi_crs_complex_tbb, test_task_run) {
  size_t p = 500;
  size_t q = 500;
  size_t r = 500;
  std::vector<Complex> lhs_in(p * q);
