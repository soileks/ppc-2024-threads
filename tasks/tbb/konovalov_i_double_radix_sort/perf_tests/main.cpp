...
TEST(konovalov_i_double_radix_sort_tbb, test_pipeline_run) {
  ...
  // Create Task
  auto testTaskSequential = std::make_shared<RadixSortTBBTaskParallel>(taskDataSeq);
...
TEST(konovalov_i_double_radix_sort_tbb, test_task_run) {
  const size_t size = 1e+6;
  std::mt19937 gen(1.0);
  std::uniform_real_distribution<> dis(0.0, 1e+5);
