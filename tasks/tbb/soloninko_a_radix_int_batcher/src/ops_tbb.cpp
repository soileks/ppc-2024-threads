...
void sol_fill_missing_tbb(std::vector<int> &res, std::vector<int> p1, std::vector<int> p2, size_t i, size_t j,
                          size_t k) {
  if (j >= p1.size()) {
    for (size_t l = k; l < p2.size(); l += 2) {
      res[i] = p2[l];
...
void sol_fill_res_tbb(std::vector<int> &res, std::vector<int> p1, std::vector<int> p2, size_t &i, size_t &j,
                      size_t &k) {
  for (i = 0; (j < p1.size()) && (k < p2.size()); i++) {
    if (p1[j] <= p2[k]) {
      res[i] = p1[j];
...
std::vector<int> sol_merge_tbb(std::vector<int> vec1, std::vector<int> vec2) {
  std::vector<int> res(vec1.size() + vec2.size());
  // size_t i = 0;
  // size_t j = 0;
  // size_t k = 0;
  ...
  tbb::parallel_for(size_t(0), res.size() - 1, [&](size_t i) {
    if (res[i] > res[i + 1]) {
      std::swap(res[i], res[i + 1]);
    }
  });
...
std::vector<int> soll_even_tbb(const std::vector<int> &p1, const std::vector<int> &p2) {
  std::vector<int> res(p1.size() / 2 + p2.size() / 2 + p1.size() % 2 + p2.size() % 2);
  size_t i = 0;
  size_t j = 0;
  size_t k = 0;
...
std::vector<int> sol_odd_tbb(const std::vector<int> &p1, const std::vector<int> &p2) {
  std::vector<int> res(p1.size() / 2 + p2.size() / 2);
  size_t i = 0;
  size_t j = 1;
  size_t k = 1;

  sol_fill_res_tbb(res, p1, p2, i, j, k);

  sol_fill_missing_tbb(res, p1, p2, i, j, k);

  return res;
}

std::vector<int> sol_batcher_sort_tbb(const std::vector<int> &p1, const std::vector<int> &p2) {
  std::vector<int> even_vec = soll_even_tbb(p1, p2);

  std::vector<int> odd_vec = sol_odd_tbb(p1, p2);

  std::vector<int> res = sol_merge_tbb(even_vec, odd_vec);

  return res;
}

bool TestTaskTBBParallelBatcherSoloninko::pre_processing() {
  internal_order_test();
...
bool TestTaskTBBParallelBatcherSoloninko::validation() {
  internal_order_test();
...
bool TestTaskTBBParallelBatcherSoloninko::run() {
  internal_order_test();

  res = sol_batcher_sort_tbb(p1, p2);
  return true;
}

bool TestTaskTBBParallelBatcherSoloninko::post_processing() {
  internal_order_test();
  std::copy(res.begin(), res.end(), reinterpret_cast<int *>(taskData->outputs[0]));
  return true;
}

bool TaskTBBSequentialBatcherSoloninko::pre_processing() {
  internal_order_test();
  // Init value for input and output
  in_vec = std::vector<int>(taskData->inputs_count[0]);
...
bool TaskTBBSequentialBatcherSoloninko::validation() {
  internal_order_test();
  // Check count elements of output
...
bool TaskTBBSequentialBatcherSoloninko::run() {
  internal_order_test();
...
bool TaskTBBSequentialBatcherSoloninko::post_processing() {
  internal_order_test();
  std::copy(res.begin(), res.end(), reinterpret_cast<int *>(taskData->outputs[0]));
  return true;
}
