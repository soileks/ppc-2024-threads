...
TEST(Safarov_N_SparseMatMultCRS_OMP, TestOne) {
  // Create data
  ...
  // Create Task
  SparseMatrixMultiplicationCRS_OMP taskOmp(taskDataOmp);
...
TEST(Safarov_N_SparseMatMultCRS_OMP, TestTwo) {
  // Create data
...
TEST(Safarov_N_SparseMatMultCRS_OMP, TestThree) {
  // Create data
  SparseMatrixCRS x;
  x.numberOfRows = 5;
  x.numberOfColumns = 5;
  ...
  std::shared_ptr<ppc::core::TaskData> taskDataOmp = std::make_shared<ppc::core::TaskData>();
  taskDataOmp->inputs.emplace_back(reinterpret_cast<uint8_t *>(&x));
  taskDataOmp->inputs.emplace_back(reinterpret_cast<uint8_t *>(&identityMatrix));
  taskDataOmp->outputs.emplace_back(reinterpret_cast<uint8_t *>(&oldX));
...
TEST(Safarov_N_SparseMatMultCRS_OMP, TestFour) {
  // Create data
  SparseMatrixCRS x;
  x.numberOfRows = 4;
  x.numberOfColumns = 5;
...
TEST(Safarov_N_SparseMatMultCRS_OMP, TestFive) {
  // Create data
  SparseMatrixCRS x;
  x.numberOfRows = 3;
  x.numberOfColumns = 4;
