...
bool SobelTaskParallel::validation() {
  internal_order_test();
  return (taskData->inputs_count.size() == 2) && (taskData->outputs_count.size() == 2);
}

bool SobelTaskParallel::pre_processing() {
  internal_order_test();
  try {
...
bool SobelTaskParallel::post_processing() {
  internal_order_test();
  try {
...
int SobelTaskParallel::clamp(int value, int min, int max) {
