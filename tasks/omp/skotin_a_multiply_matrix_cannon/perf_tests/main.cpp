...
TEST(Skotin_A_Multiply_Matrix_Cannon_Seq, test_pipeline_run) {
  ...
  auto matrixAData = SkotinOMPPerfTests::createMatrixData(matrixSize, 1.0);
  auto matrixBData = SkotinOMPPerfTests::createMatrixData(matrixSize, 2.0);
  ...
  auto task = std::make_shared<SkotinMatrixMultiplicationOMPParallel>(taskData);
