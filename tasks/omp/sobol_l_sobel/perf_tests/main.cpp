...
TEST(sobol_l_sobel_omp, test_pipeline_run) {
  int width = 1000;
  int height = 1000;
  ...
  // Создание задачи
  std::shared_ptr<sobol::Sobel_omp> sobel = std::make_shared<sobol::Sobel_omp>(taskData, width, height);
...
TEST(sobol_l_sobel_omp, test_task_run) {
  int width = 3000;
  int height = 3000;
