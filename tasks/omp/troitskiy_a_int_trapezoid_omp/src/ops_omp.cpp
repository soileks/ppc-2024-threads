...
bool TestTaskSequentialtroitskIntegralTrapezoid::pre_processing() {
  internal_order_test();
...
bool TestTaskSequentialtroitskIntegralTrapezoid::validation() {
  internal_order_test();
  // Check count elements of output
  return taskData->inputs_count[0] == 4 && taskData->inputs_count[1] == 2 && taskData->outputs_count[0] == 1;
}

bool TestTaskSequentialtroitskIntegralTrapezoid::run() {
  internal_order_test();
  res = trapezoidal_integral(a1, b1, a2, b2, n1, n2, fun);
  return true;
}

bool TestTaskSequentialtroitskIntegralTrapezoid::post_processing() {
  internal_order_test();
  reinterpret_cast<double *>(taskData->outputs[0])[0] = res;
  return true;
}

bool TestOMPTaskParalleltroitskIntegralTrapezoid::pre_processing() {
  internal_order_test();
  // Init vectors
  a1 = reinterpret_cast<double *>(taskData->inputs[0])[0];
  b1 = reinterpret_cast<double *>(taskData->inputs[0])[1];
...
bool TestOMPTaskParalleltroitskIntegralTrapezoid::validation() {
  internal_order_test();
  // Check count elements of output
  return taskData->inputs_count[0] == 4 && taskData->inputs_count[1] == 2 && taskData->outputs_count[0] == 1;
}

bool TestOMPTaskParalleltroitskIntegralTrapezoid::run() {
  internal_order_test();
  res = trapezoidal_integral_omp(a1, b1, a2, b2, n1, n2, fun);
  return true;
}

bool TestOMPTaskParalleltroitskIntegralTrapezoid::post_processing() {
  internal_order_test();
  reinterpret_cast<double *>(taskData->outputs[0])[0] = res;
  return true;
}
