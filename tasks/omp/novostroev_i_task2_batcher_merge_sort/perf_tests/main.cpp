...
TEST(novostroev_i_batcher_sort_perf_omp, test_pipeline_run) {
  ...
  auto testOmp = std::make_shared<BatcherMergeSortOMP>(taskDataSeq);
...
TEST(novostroev_i_batcher_sort_perf_omp, test_task_run) {
  std::vector<int> vect = RandomVector(100000);
  std::vector<int> result(vect.size(), 0);
