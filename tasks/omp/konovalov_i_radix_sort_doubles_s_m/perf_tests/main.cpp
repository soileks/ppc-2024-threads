...
TEST(konovalov_i_double_radix_sort_omp, test_pipeline_run) {
  const size_t size = 1e+6;
  std::mt19937 gen(1.0);
  std::uniform_real_distribution<> dis(0.0, 1e+5);
  ...
  // Create Task
  auto testTaskSequential = std::make_shared<RadixSortOMPTaskParallel>(taskDataSeq);
...
TEST(konovalov_i_double_radix_sort_omp, test_task_run) {
  const size_t size = 1e+6;
  std::mt19937 gen(1.0);
  std::uniform_real_distribution<> dis(0.0, 1e+5);
