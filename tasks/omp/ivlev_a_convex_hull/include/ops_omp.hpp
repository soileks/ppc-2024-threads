...
namespace ivlev_a_omp {
  ...
  public:
    explicit ConvexHullOMPTaskSequential(std::shared_ptr<ppc::core::TaskData> taskData_) : Task(std::move(taskData_)) {}
  ...
  private:
    ...
    static std::vector<std::pair<size_t, size_t>> Convex_Hull_tmp(size_t p, size_t last, size_t n, const std::vector<std::pair<size_t, size_t>>& component_);
    static std::vector<std::pair<size_t, size_t>> Convex_Hull(const std::vector<std::pair<size_t, size_t>>& component_);
  };

  size_t rotation(const std::pair<ptrdiff_t, ptrdiff_t>& a, const std::pair<ptrdiff_t, ptrdiff_t>& b, const std::pair<ptrdiff_t, ptrdiff_t>& c);
}
