...
bool sokolova_omp::GaussFilterHorizontalOmp::pre_processing() {
  internal_order_test();
...
bool sokolova_omp::GaussFilterHorizontalOmp::validation() {
  internal_order_test();
...
bool sokolova_omp::GaussFilterHorizontalOmp::run() {
  internal_order_test();
  applyKernel();
  return true;
}

bool sokolova_omp::GaussFilterHorizontalOmp::post_processing() {
  internal_order_test();
  std::memcpy(output, image.data(), width * height * 3);
  return true;
}

void sokolova_omp::GaussFilterHorizontalOmp::makeKernel(float sigma) {
...
void sokolova_omp::GaussFilterHorizontalOmp::applyKernel() {
...
sokolova_omp::Color sokolova_omp::GaussFilterHorizontalOmp::calculateNewPixelColor(size_t x, size_t y) {
  sokolova_omp::Color resultColor;
  uint32_t radius = kSize / 2;
  float newR = 0;
  float newG = 0;
  float newB = 0;
