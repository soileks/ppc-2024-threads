...
TEST(borovkov_s_omp_perf_tests, test_900x900) {
  // Create data
  size_t size = 900;
  size_t block = 450;
  ...
  auto inputMatrOne = BorovkovOmp::getRandomSquareMatrix(size, minVal, maxVal);
  auto inputMatrTwo = BorovkovOmp::getRandomSquareMatrix(size, minVal, maxVal);
  ...
  auto resSeq = BorovkovOmp::CannonMatrixMultSeq(inputMatrOne, inputMatrTwo, size, block);

  // Create Task
  auto testTaskOmp = std::make_shared<BorovkovOmp::BorovkovCanMultMatrOmp>(taskDataOmp);
  ...
  for (size_t i = 0; i < resSize; ++i) {
    ASSERT_TRUE(BorovkovOmp::isEqual(resSeq[i], outputMatr[i]));
  }
}

TEST(borovkov_s_omp_perf_tests, test_800x800) {
  // Create data
  size_t size = 800;
  size_t block = 400;
