...
#pragma omp for nowait
    for (std::size_t i = 0; i < n; i++) local_count[(arr[i] / exp) % 10]++;

#pragma omp critical
    for (int i = 0; i < 10; i++) count[i] += local_count[i];
  }
...
#pragma omp parallel for
  ...
  a1.assign(merged.begin(), merged.begin() + half_n);
  a2.assign(merged.begin() + half_n, merged.end());
