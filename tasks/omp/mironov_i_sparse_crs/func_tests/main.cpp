...
TEST(mironov_i_sparse_crs_omp, Test1Static) {
...
TEST(mironov_i_sparse_crs_omp, Test2Static) {
  std::vector<double> C(3 * 3, 0.0);
...
TEST(mironov_i_sparse_crs_omp, TestEdin34) {
  std::vector<double> A = {2, 0, 2, 0, 0, 1, 0, 2, -2, 0, 3, 0};
  int n = 3;
  int m = A.size() / n;
  std::vector<double> B(m * m);
  MironovIOMP::genrateEdMatrix(B.data(), m);
...
TEST(mironov_i_sparse_crs_omp, TestEdin43) {
  std::vector<double> A = {2, 0, 2, 0, 0, 1, 0, 2, -2, 0, 3, 0};
  int n = 4;
  int m = A.size() / n;
  std::vector<double> B(m * m);
...
TEST(mironov_i_sparse_crs_omp, TestRandom) {
  int n = 15;
  int m = 15;
  int k = 15;
  double ro = 0.3;
