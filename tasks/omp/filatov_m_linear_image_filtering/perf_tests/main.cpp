...
class filatov_m_linear_image_filtering_perf_omp : public ::testing::Test {
 protected:
...
TEST_F(filatov_m_linear_image_filtering_perf_omp, test_pipeline_run_with_red_image) {
  SetUpWH(512, 512);
  CommonSetUp();
  FillChannel(0, 111);
...
TEST_F(filatov_m_linear_image_filtering_perf_omp, test_task_run_with_blue_image) {
  SetUpWH(512, 512);
  CommonSetUp();
  FillChannel(1, 255);
  CreateGaussFilterHorizontal();
