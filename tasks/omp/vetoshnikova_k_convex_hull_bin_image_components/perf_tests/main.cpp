...
TEST(vetoshnikova_k_hull_bin_image_omp, test_pipeline_run) {
  int h = 200;
  int w = 200;
  // Create data
  ...
  // Create Task
  auto testTaskSequential = std::make_shared<vetoshnikova_omp::ConstructingConvexHullOMP>(taskDataSeq);
...
TEST(vetoshnikova_k_hull_bin_image_omp, test_task_run) {
  int h = 200;
  int w = 200;
  // Create data
  std::vector<int> out(10 * w * h);
